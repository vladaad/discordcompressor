...
func encFDKaac(outFilename string, bitrate float64, eOptions *settings.AudioEncoder, input io.ReadCloser) {
	var options []string
	encoderSettings := strings.Split(eOptions.Options, " ")

	// Encoding options
	if eOptions.UsesBitrate {
		options = append(options, "-b", strconv.FormatFloat(bitrate, 'f', -1, 64))
	}
	if eOptions.Options != "" {
